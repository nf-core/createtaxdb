...
# `nf-core/createtaxdb`: Contributing guidelines

Hi there!
Thanks for taking an interest in improving nf-core/createtaxdb.

This page describes the recommended nf-core way to contribute to both nf-core/createtaxdb and nf-core pipelines in general, including:

- [General contribution guidelines](#general-contribution-guidelines): common procedures or guides across all nf-core pipelines.
- [Pipeline-specific contribution guidelines](#pipeline-specific-contribution-guidelines): procedures or guides specific to the development conventions of nf-core/createtaxdb.

> [!NOTE]
> If you need help using or modifying nf-core/createtaxdb, ask on the nf-core Slack [#createtaxdb](https://nfcore.slack.com/channels/createtaxdb) channel ([join our Slack here](https://nf-co.re/join/slack)).
...
- [ ] Check whether a GitHub [issue](https://github.com/nf-core/createtaxdb/issues) about your idea already exists. If an issue does not exist, create one so that others are aware you are working on it.
- [ ] [Fork](https://help.github.com/en/github/getting-started-with-github/fork-a-repo) the [nf-core/createtaxdb repository](https://github.com/nf-core/createtaxdb) to your GitHub account.
...
For further information and help, see the [nf-core/createtaxdb documentation](https://nf-co.re/createtaxdb/usage) or ask on the nf-core [#createtaxdb](https://nfcore.slack.com/channels/createtaxdb) Slack channel ([join our Slack here](https://nf-co.re/join/slack)).

### GitHub Codespaces

You can contribute to nf-core/createtaxdb without installing a local development environment on your machine by using [GitHub Codespaces](https://github.com/codespaces).
...
To get started, open the repository in [Codespaces](https://github.com/nf-core/createtaxdb/codespaces).
...
nf-core semi-standardises how you write code and other contributions to make the nf-core/createtaxdb code and processing logic more understandable for new contributors and to ensure quality.
...
### Adding new profiler building tool workflow
...
- [ ] Added parameters to `nextflow.config`
  ...
  - [ ] If it's your first contribution, add, or move yourself to the contributor list in the `manifest`!
...
- [ ] Update Documentation
  - [ ] `nf-core pipelines schema build` has been run and updated
    - [ ] All additional tool specific pipeline parameters have an additional help entry with the `Modifies tool parameter(s)` quote block
...
### Reviewing new profiler building tool workflow

This checklist is the same as the previous section, but in the order that GitHub normally displays the files.

- [ ] `assets/createtaxdb-metromap-diagram*`
  - [ ] (Optional) updated
- [ ] `conf/modules.conf`
  - [ ] Added `withName:` block
  - [ ] Added `ext.args = ${params.<profiler>_build_params}`
...
- [ ] `subworkflows/local/preprocessing/main.nf`
  ...
  - [ ] If relevant, update the subworkflow's if/else (conatenate into a single ref) sections for either DNA or AA FASTA preprocessing
  - [ ] Format with VSCode Nextflow extension
- [ ] `subworkflows/utils_nfcore_createtaxdb_pipeline`
  - [ ] If necessary, added any profiler-specific parameter validation checks to `utils_nfcore_createtaxdb_pipeline` and possible at the top of `createtaxdb.nf`
  - [ ] Add citation to the toolCitation/BibliographyText functions in `utils_nfcore_createtaxdb_pipeline` subworkflow
    - [ ] Added in-text citation
    - [ ] Added bibliography (citation style: APA 7th edition)
