...
- Statistics and run information
...
- Postprocessing
  - [Downstream pipeline](#downstream-samplesheets) input samplesheets.
...
[PIGZ](https://github.com/madler/pigz) is used to decompress gzipped input FASTA files.
...
- `unpigz/`
  - `ungzipped/`
    - `*fna`: Uncompressed input FASTA file

</details>

Will only be present if you run with `--save_uncompressed_fastas`.
...
- `unpigz/`
  - `ungzipped/`
    - `*fna`: Concatenated input FASTA file
...
This ensures that the database files are not lost if the pipeline is rerun, and allows you to share the database files with other users.
